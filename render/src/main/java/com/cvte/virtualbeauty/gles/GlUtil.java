...
public class GlUtil {
    ...
    public static int createProgramFromAssets(Context context, String vertexPath, String fragmentPath) {
        BufferedReader vbr = null, fbr = null;
        StringBuilder sb = new StringBuilder();
        String line = null;
        String vertexSource = null, fragmentSource = null;
        try {
            vbr = new BufferedReader(new InputStreamReader(context.getAssets().open("shaders/" + vertexPath)));
            while ((line = vbr.readLine()) != null) {
                sb.append(line).append("\n");
            }
            vertexSource = sb.toString();
            sb.setLength(0);
            fbr = new BufferedReader(new InputStreamReader(context.getAssets().open("shaders/" + fragmentPath)));
            while ((line = fbr.readLine()) != null) {
                sb.append(line).append("\n");
            }
            fragmentSource = sb.toString();
        } catch (IOException e) {
            LogUtil.e(TAG, "Could not open shader file");
            return 0;
        } finally {
            if (vbr != null) {
                try {
    ...
    public static int loadShader(int shaderType, String source) {
        if (TextUtils.isEmpty(source)) throw new RuntimeException("shader source can't be null");
    ...
    public static void loadTexImage2DFromAssets(Context context, String path) {
        try {
            InputStream is = context.getAssets().open(path);
            Bitmap bmp = BitmapFactory.decodeStream(is);
            GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, bmp, 0);
        } catch (IOException e) {
            LogUtil.e(TAG, "load " + path + " from assets failed");
        }
    }
    ...
    /**
     * Writes GL version info to the LogUtil.
     */
