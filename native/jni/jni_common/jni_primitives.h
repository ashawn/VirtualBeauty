...
#define CLASSNAME_VISION_DET_RET "com/cvte/virtualbeauty/dlib/VisionDetRet"
#define CONSTSIG_VISION_DET_RET "()V"

#define CLASSNAME_DETECTORRET "com/cvte/virtualbeauty/dlib/DetectorRet"
#define CONSTSIG_DETECTORRET "()V"

#define CLASSNAME_FACE_DET "com/cvte/virtualbeauty/dlib/FaceDet"
#define CLASSNAME_PEDESTRIAN_DET "com/cvte/virtualbeauty/dlib/PedestrianDet"
...
class JNI_DetectorRet {
public:
    JNI_DetectorRet(JNIEnv *env) {
        jclass clazz = env->FindClass(CLASSNAME_DETECTORRET);
        CHECK_NOTNULL(clazz);

        jID_detected = env->GetFieldID(clazz, "mDetected", "Z");
        jID_spend_time = env->GetFieldID(clazz, "mSpendTime" , "J");
        jMethodID_addPoint = env->GetMethodID(clazz, "addPoint", "(III)Z");
        jMethodID_setRotateMatrixAtPosition = env->GetMethodID(clazz,"setRotateMatrixAtPosition","(FI)V");
    }

    void setDetected(JNIEnv *env, jobject &obj, bool flag) {
        env->SetBooleanField(obj, jID_detected, flag);
    }

    void setSpendTime(JNIEnv *env, jobject &obj, long time) {
        env->SetLongField(obj,jID_spend_time,time);
    }

    void addPoint(JNIEnv *env, jobject &obj, int pEnum, int x, int y) {
        env->CallBooleanMethod(obj, jMethodID_addPoint, pEnum, x, y);
    }

    static jobject createJObject(JNIEnv *env) {
        jclass clazz = env->FindClass(CLASSNAME_DETECTORRET);
        jmethodID mid =
                env->GetMethodID(clazz, "<init>", CONSTSIG_DETECTORRET);
        return env->NewObject(clazz, mid);
    }

    void setRotateMatrixAtPosition(JNIEnv *env, jobject  &obj,float value,int pos){
        env->CallVoidMethod(obj,jMethodID_setRotateMatrixAtPosition,value,pos);
    }

private:
    jfieldID jID_detected;
    jfieldID jID_spend_time;
    jmethodID jMethodID_addPoint;
    jmethodID jMethodID_setRotateMatrixAtPosition;
};
