...
void PSGray::init() {
    ...
    m_Program = glutil::get_program_by_type(PS_GRAY);
...
void PSGray::setTextureId(GLuint TextureId) {
    mTEXTURE = TextureId;
}

GLuint PSGray::getFrameTextureId() {
    return mFrameTexture;
}

void PSGray::draw() {
    LOG(INFO) << "PSGray draw";
...
void PSGray::loadShader(GLuint program) {
    m_Program = program;
}

void PSGray::update(int width, int height) {
    mWidth = width;
    mHeight = height;
    glBindFramebuffer(GL_FRAMEBUFFER,m_Frame_GL_Buffer);
