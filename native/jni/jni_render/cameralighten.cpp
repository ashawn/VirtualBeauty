...
void CameraLighten::init() {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(CAMERA_INDICES_DATA) * sizeof(GLubyte),
                 CAMERA_INDICES_DATA,
                 GL_STATIC_DRAW);
    ...
    m_Program = glutil::get_program_by_type(LIGHTEN);
...
GLuint CameraLighten::getFrameTextureId() {
    return mFrameTexture;
}

void CameraLighten::setCameraTextureId(GLuint CameraTexture) {
    mCameraTEXTURE = CameraTexture;
}

void CameraLighten::setTextureId(GLuint TextureId) {
    mTEXTURE = TextureId;
}

void CameraLighten::draw() {
    LOG(INFO) << "CameraLighten draw";
...
void CameraLighten::loadShader(GLuint program) {
    m_Program = program;
}

void CameraLighten::update(int width, int height) {
    mWidth = width;
    mHeight = height;
    glBindFramebuffer(GL_FRAMEBUFFER, m_Frame_GL_Buffer);
