//
// Created by ashawn on 11/05/2017.
//
...
static void obj_write_obj(const obj *O, const char *mobj,
                          const char *mtl, int prec)
{
    FILE *fout;

    if ((fout = fopen(mobj, "w")))
    {
        char formv[256];
        char formt[256];
        char formn[256];
...
void obj_write(const obj *O, const char *mobj, const char *mtl, int prec)
{
    assert(O);

    if (mobj) obj_write_obj(O, mobj, mtl, prec);
