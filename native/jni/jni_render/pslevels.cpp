...
void PSLevels::init() {
    ...
    m_Program = glutil::get_program_by_type(PS_LEVELS);
    m_Position_Handle = glGetAttribLocation(m_Program, "position");
    m_TexCoord_Handle = glGetAttribLocation(m_Program, "texCoord");
    m_TextureHandle = glGetUniformLocation(m_Program,"image");

    mWidth = 0;
    mHeight = 0;
}

void PSLevels::setTextureId(GLuint TextureId) {
    mTEXTURE = TextureId;
}

GLuint PSLevels::getFrameTextureId() {
    return mFrameTexture;
}

void PSLevels::draw() {
    LOG(INFO) << "PSLevels draw";
...
void PSLevels::loadShader(GLuint program) {
    m_Program = program;
}

void PSLevels::update(int width, int height) {
    mWidth = width;
    mHeight = height;
    glBindFramebuffer(GL_FRAMEBUFFER,m_Frame_GL_Buffer);
